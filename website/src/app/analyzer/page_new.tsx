                    ...
                    {selectedConversation.messages.map((message) => (
                      ...
                      >
                        <div className={`max-w-xs lg:max-w-md ${
                          message.sender === 'customer' ? 'bg-gray-100' : 'bg-green-100'
                        } rounded-lg p-3`}>
                          <div className="flex items-center gap-2 mb-1">
                          ...
                          {message.sentiment && (
                            <div className={`mt-2 inline-flex items-center gap-1 px-2 py-1 rounded text-xs ${getSentimentColor(message.sentiment.predictedClass)}`}>
                              {getSentimentIcon(message.sentiment.predictedClass)}
                              <span>{getSentimentLabel(message.sentiment.predictedClass)}</span>
                              <span className="text-xs opacity-75">
                                ({Math.round(message.sentiment.confidence * 100)}%)
                              </span>
                            </div>
                          )}
                        </div>
